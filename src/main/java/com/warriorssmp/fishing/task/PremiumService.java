 ...
 *   - the player has the "premiumfishing" permission (e.g. from a rank plugin)
...
public final class PremiumService {
    ...
    public PremiumService(FishingPlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean isPremium(Player player) {
        return player.isOp() || player.hasPermission("premiumfishing") || granted.contains(player.getUniqueId());
    }
    ...
    public String describeStatus(OfflinePlayer target) {
        ...
        if (target.getPlayer() != null && target.getPlayer().hasPermission("premiumfishing")) return "§dPermission";
