...
public final class LuckyStrikeService {
    ...
    public LuckyStrikeService(FishingPlugin plugin, GatherConfig config, DataStore dataStore, EconomyService economy) {
        this.plugin = plugin;
        this.config = config;
    ...
    public boolean roll(Player player, PlayerGatherData data, Material material, int tierOfMaterial) {
        ...
        if (data.luckyStrikeBoostExpiry > System.currentTimeMillis()) {
            chance *= 1.10; // the Fishing Buff (paid or milestone-earned) is a relative +10%
        }
        ...
        player.sendActionBar(Component.text("§e🌟 RARE CATCH! §7x2 " + resourceName));
        player.showTitle(Title.title(
                Component.text("§e🌟 RARE CATCH!"),
