package com.warriorssmp.fishing.data;
...
public final class DataStore {
    ...
    public DataStore(FishingPlugin plugin) {
        this.plugin = plugin;
    ...
    private PlayerGatherData load(UUID uuid) {
        ...
        data.catchesTowardMilestone = yml.getInt("catches-toward-milestone", 0);
        data.fishdexRewardClaimed = yml.getBoolean("fishdex-reward-claimed", false);
        for (String matName : yml.getStringList("fishdex-discovered")) {
            Material m = Material.matchMaterial(matName);
            if (m != null) data.fishdexDiscovered.add(m);
        }
        var catchCountsSection = yml.getConfigurationSection("fishdex-catch-counts");
        if (catchCountsSection != null) {
            for (String matName : catchCountsSection.getKeys(false)) {
                Material m = Material.matchMaterial(matName);
                if (m != null) data.fishdexCatchCounts.put(m, catchCountsSection.getInt(matName, 0));
            }
        }
    ...
    public void save(PlayerGatherData data) {
        ...
        yml.set("catches-toward-milestone", data.catchesTowardMilestone);
        yml.set("fishdex-reward-claimed", data.fishdexRewardClaimed);
        List<String> fishdexOut = new ArrayList<>();
        for (Material m : data.fishdexDiscovered) fishdexOut.add(m.name());
        yml.set("fishdex-discovered", fishdexOut);
        for (var entry : data.fishdexCatchCounts.entrySet()) {
            yml.set("fishdex-catch-counts." + entry.getKey().name(), entry.getValue());
        }
        ...
        try {
            ...
            plugin.getLogger().warning("Failed to save Angler data for " + data.uuid + ": " + e.getMessage());
        }
    }
